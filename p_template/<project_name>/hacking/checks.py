...
Guidelines for writing new hacking checks
 ...
 - Use only for <Project_name> specific tests. OpenStack general tests
 ...
 - Add test cases for each new rule to
   <project_name>/tests/test_hacking.py

"""

# NOTE(thangp): Ignore N323 pep8 error caused by importing <project_name> objects
UNDERSCORE_IMPORT_FILES = ['./<project_name>/objects/__init__.py']
...
class CheckLoggingFormatArgs(BaseASTChecker):
    ...
    LOG.debug("<Project_name> %s caught fire and is at %d degrees C and climbing.",
              ('<project_name>1', 500))
...
def validate_log_translations(logical_line, filename):
    ...
    if "<project_name>/tests" in filename:
        return
...
def check_no_print_statements(logical_line, filename, noqa):
    # The files in <project_name>/cmd do need to use 'print()' so
    # we don't need to check those files.  Other exemptions
    # should use '# noqa' to avoid failing here.
    if "<project_name>/cmd" not in filename and not noqa:
...
def no_test_log(logical_line, filename, noqa):
    if "<project_name>/tests" not in filename or noqa:
        return
    # Skip the "integrated" tests for now
    if "<project_name>/tests/unit/integrated" in filename:
        return
