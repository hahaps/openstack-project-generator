...
"""Unit tests for `<project_name>.wsgi`."""
...
class ExceptionTest(test.TestCase):
    ...
    def _wsgi_app(self, inner_app):
        ...
        # test suite only.
        from <project_name>.api.middleware import fault
        return fault.FaultWrapper(inner_app)

    def _do_test_exception_safety_reflected_in_faults(self, expose):
        class ExceptionWithSafety(exception.<Project_name>Exception):
            safe = expose
    ...
    def test_non_<project_name>_notfound_exception_mapping(self):
    ...
    def test_non_<project_name>_exception_mapping(self):
        class ExceptionWithCode(Exception):
            code = 417
    ...
    @mock.patch('<project_name>.i18n.translate')
    def test_<project_name>_exception_with_localized_explanation(self, mock_t9n):
        msg = 'My Not Found'
        ...
        @webob.dec.wsgify
        def fail(req):
            class My<Project_name>NotFound(exception.NotFound):
                def __init__(self):
                    self.msg = message
                    self.safe = True
            raise My<Project_name>NotFound()
