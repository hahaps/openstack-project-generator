...
function usage {
  echo "Usage: $0 [OPTION]..."
  echo "Run <Project_name>'s test suite(s)"
  echo ""
...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./<project_name>/tests"
    fi
  ...
  if [ setup.cfg -nt <project_name>.egg-info/entry_points.txt ]
  then
...
function run_pep8 {
  ...
  ${wrapper} tools/check_exec.py <project_name> || exit 1
}
