...
from <project_name>.i18n import _, _LI, _LW
...
from <project_name>.wsgi import eventlet_server as wsgi
...
service_opts = [
    ...
    cfg.StrOpt('osapi_<project_name>_listen',
               default="0.0.0.0",
               help='IP address on which OpenStack <Project_name> API listens'),
    cfg.IntOpt('osapi_<project_name>_listen_port',
               default=8760,
               min=1, max=65535,
               help='Port on which OpenStack <Project_name> API listens'),
    cfg.IntOpt('osapi_<project_name>_workers',
               help='Number of workers for OpenStack <Project_name> API service. '
...
def setup_profiler(binary, host):
    if CONF.profiler.profiler_enabled:
        _notifier = osprofiler.notifier.create(
            ...
            rpc.TRANSPORT, "<project_name>", binary, host)
        osprofiler.notifier.set(_notifier)
        LOG.warning(
            _LW("OSProfiler is enabled.\nIt means that person who knows "
                "any of hmac_keys that are specified in "
                "/etc/<project_name>/api-paste.ini can trace his requests. \n"
                "In real life only operator can read this file so there "
                "is no security issue. Note that even if person can "
                "trigger profiler, only admin user can retrieve trace "
                "information.\n"
                "To disable OSprofiler set in <project_name>.conf:\n"
                "[profiler]\nenabled=false"))
    else:
...
class Service(service.Service):
    ...
    def create(cls, host=None, binary=None, topic=None, manager=None,
        ...
        :param topic: defaults to bin_name - '<project_name>-' part
        ...
        if not topic:
            topic = binary
        if not manager:
            subtopic = topic.rpartition('<project_name>-')[2]
