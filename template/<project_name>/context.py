...
"""RequestContext: context for requests that persist through all of <project_name>."""
...
from <project_name> import policy

context_opts = [
    cfg.StrOpt('<project_name>_internal_tenant_project_id',
               default=None,
               help='ID of the project which will be used as the <Project_name> '
                    'internal tenant.'),
    cfg.StrOpt('<project_name>_internal_tenant_user_id',
               default=None,
               help='ID of the user to be used in <manager_service_name> operations as the '
                    '<Project_name> internal tenant.'),
]
...
class RequestContext(context.RequestContext):
    ...
    # tenant/user whereas the <Project_name> version uses project_id/user_id.
    # NOTE(adrienverge): The <Project_name> version of RequestContext now uses
    # tenant/user internally, so it is compatible with context-aware code from
    # openstack/common. We still need this shim for the rest of <Project_name>'s
    # code.
    @property
    def project_id(self):
        return self.tenant
...
def get_internal_tenant_context():
    """Build and return the <Project_name> internal tenant context object

    This request context will only work for internal <Project_name> operations. It will
    not be able to make requests to remote services. To do so it will need to
    use the keystone client to get an auth_token.
    """
    project_id = CONF.<project_name>_internal_tenant_project_id
    user_id = CONF.<project_name>_internal_tenant_user_id
