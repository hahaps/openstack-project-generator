...
if __name__ == "__main__":
    opt_file = open("<project_name>/opts.py", 'a')
    opt_dict = {}
    dir_trees_list = []
    ...
    for atree in dir_trees_list:
        ...
        if atree == "<project_name>/config/generate_<project_name>_opts.py":
            continue
    ...
    for key in opt_dict:
        ...
        for aline in afile:
            ...
            if exists != -1:
                ...
                if (exists != -1) or (key == '<project_name>_<manager_service_name>_configuration'):
                    continue
