...
class ContextTestCase(test.TestCase):
    ...
    @mock.patch('<project_name>.context.CONF')
    def test_<project_name>_internal_context(self, mock_conf):
        project_id = 'ec729e9946bc43c39ece6dfa7de70eea'
        user_id = 'c466a48309794261b64a4f02cfcc3d64'
        mock_conf.<project_name>_internal_tenant_project_id = project_id
        mock_conf.<project_name>_internal_tenant_user_id = user_id
        ctx = context.get_internal_tenant_context()
        self.assertEqual(user_id, ctx.user_id)
        self.assertEqual(project_id, ctx.project_id)

    @mock.patch('<project_name>.context.CONF')
    def test_<project_name>_internal_context_missing_user(self, mock_conf):
        project_id = 'ec729e9946bc43c39ece6dfa7de70eea'
        user_id = None
        mock_conf.<project_name>_internal_tenant_project_id = project_id
        mock_conf.<project_name>_internal_tenant_user_id = user_id
    ...
    @mock.patch('<project_name>.context.CONF')
    def test_<project_name>_internal_context_missing_project(self, mock_conf):
        project_id = None
        user_id = 'c466a48309794261b64a4f02cfcc3d64'
        mock_conf.<project_name>_internal_tenant_project_id = project_id
        mock_conf.<project_name>_internal_tenant_user_id = user_id
    ...
    @mock.patch('<project_name>.context.CONF')
    def test_<project_name>_internal_context_missing_all(self, mock_conf):
        project_id = None
        user_id = None
        mock_conf.<project_name>_internal_tenant_project_id = project_id
        mock_conf.<project_name>_internal_tenant_user_id = user_id
