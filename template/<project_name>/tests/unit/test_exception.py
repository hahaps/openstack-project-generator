...
class FakeNotifier(object):
    """Acts like the <project_name>.openstack.common.notifier.api module."""
    ERROR = 88
...
class <Project_name>ExceptionTestCase(test.TestCase):
    def test_default_error_msg(self):
        class Fake<Project_name>Exception(exception.<Project_name>Exception):
            message = "default message"

        exc = Fake<Project_name>Exception()
        self.assertEqual('default message', six.text_type(exc))

    def test_error_msg(self):
        self.assertEqual('test',
                         six.text_type(exception.<Project_name>Exception('test')))

    def test_default_error_msg_with_kwargs(self):
        class Fake<Project_name>Exception(exception.<Project_name>Exception):
            message = "default message: %(code)s"

        exc = Fake<Project_name>Exception(code=500)
    ...
    def test_error_code_from_kwarg(self):
        ...
        exc = Fake<Project_name>Exception(code=404)
        self.assertEqual(404, exc.kwargs['code'])

    def test_error_msg_is_exception_to_string(self):
        msg = 'test message'
        exc1 = Exception(msg)
        exc2 = exception.<Project_name>Exception(exc1)
        self.assertEqual(msg, exc2.msg)

    def test_exception_kwargs_to_string(self):
        msg = 'test message'
        exc1 = Exception(msg)
        exc2 = exception.<Project_name>Exception(kwarg1=exc1)
        self.assertEqual(msg, exc2.kwargs['kwarg1'])

    def test_message_in_format_string(self):
        class Fake<Project_name>Exception(exception.<Project_name>Exception):
            message = 'Fake<Project_name>Exception: %(message)s'

        exc = Fake<Project_name>Exception(message='message')
        self.assertEqual('Fake<Project_name>Exception: message', six.text_type(exc))

    def test_message_and_kwarg_in_format_string(self):
        class Fake<Project_name>Exception(exception.<Project_name>Exception):
            message = 'Error %(code)d: %(message)s'

        exc = Fake<Project_name>Exception(message='message', code=404)
    ...
    def test_message_is_exception_in_format_string(self):
        ...
        msg = 'test message'
        exc1 = Exception(msg)
        exc2 = Fake<Project_name>Exception(message=exc1)
