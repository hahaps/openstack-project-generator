...
from <project_name>.hacking import checks
from <project_name> import test


@ddt.ddt
class HackingTestCase(test.TestCase):
    """This class tests the hacking checks in <project_name>.hacking.checks

    This class ensures that <Project_name>'s hacking checks are working by passing
    ...
    def test_no_translate_debug_logs(self):
        self.assertEqual(1, len(list(checks.no_translate_debug_logs(
            "LOG.debug(_('foo'))", "<project_name>/scheduler/foo.py"))))

        self.assertEqual(0, len(list(checks.no_translate_debug_logs(
            "LOG.debug('foo')", "<project_name>/scheduler/foo.py"))))

        self.assertEqual(0, len(list(checks.no_translate_debug_logs(
            "LOG.info(_('foo'))", "<project_name>/scheduler/foo.py"))))

    def test_check_explicit_underscore_import(self):
        self.assertEqual(1, len(list(checks.check_explicit_underscore_import(
            "LOG.info(_('My info message'))",
            "<project_name>.tests.unit/other_files.py"))))
        self.assertEqual(1, len(list(checks.check_explicit_underscore_import(
            "msg = _('My message')",
            "<project_name>.tests.unit/other_files.py"))))
        ...
        self.assertEqual(0, len(list(checks.check_explicit_underscore_import(
            ...
            "<project_name>.tests.unit/other_files2.py"))))
        self.assertEqual(0, len(list(checks.check_explicit_underscore_import(
            "msg = _('My message')",
            "<project_name>.tests.unit/other_files2.py"))))
        self.assertEqual(0, len(list(checks.check_explicit_underscore_import(
            "_ = translations.ugettext",
            "<project_name>.tests.unit/other_files3.py"))))
        self.assertEqual(0, len(list(checks.check_explicit_underscore_import(
            "msg = _('My message')",
            "<project_name>.tests.unit/other_files3.py"))))
        ...
        self.assertEqual(0, len(list(checks.check_explicit_underscore_import(
            ...
            "<project_name>.tests.unit/other_files4.py"))))
    ...
    def test_logging_format_args(self):
        ...
        code = """
               ...
               LOG.debug("<Project_name> %s caught fire and is at %d degrees C and"
                         " climbing.", '<project_name>1', 500)
               """
        ...
        code = """
               import logging
               LOG = logging.getLogger()
               LOG.{0}("<Project_name> %s caught fire and is at %d degrees C and "
                      "climbing.", ('<project_name>1', 500))
               """
        ...
        code = """
               import logging
               LOG = logging.getLogger()
               LOG.log(logging.DEBUG, "<Project_name> %s caught fire and is at %d"
                       " degrees C and climbing.", ('<project_name>1', 500))
               """
    ...
    def test_validate_log_translations(self):
        ...
        self.assertEqual(0, len(list(checks.validate_log_translations(
            "LOG.info('foo')", "<project_name>/tests/foo.py"))))
        self.assertEqual(0, len(list(checks.validate_log_translations(
    ...
    def test_no_print_statements(self):
        self.assertEqual(0, len(list(checks.check_no_print_statements(
            ...
            "<project_name>/file.py", False))))
        self.assertEqual(1, len(list(checks.check_no_print_statements(
            "print('My print statement')",
            "<project_name>/file.py", False))))
        self.assertEqual(0, len(list(checks.check_no_print_statements(
            "print('My print statement in <project_name>/cmd, which is ok.')",
            "<project_name>/cmd/file.py", False))))
        self.assertEqual(0, len(list(checks.check_no_print_statements(
            "print('My print statement that I just must have.')",
            "<project_name>.tests.unit/file.py", True))))
        self.assertEqual(1, len(list(checks.check_no_print_statements(
            "print ('My print with space')",
            "<project_name>/<project_name>/anotherFile.py", False))))
    ...
    @ddt.unpack
    @ddt.data(
        (1, 'LOG.info', "<project_name>/tests/unit/fake.py", False),
        (1, 'LOG.warning', "<project_name>/tests/fake.py", False),
        (1, 'LOG.error', "<project_name>/tests/fake.py", False),
        (1, 'LOG.exception', "<project_name>/tests/fake.py", False),
        (1, 'LOG.debug', "<project_name>/tests/fake.py", False),
        (0, 'LOG.info.assert_called_once_with', "<project_name>/tests/fake.py", False),
        (0, 'some.LOG.error.call', "<project_name>/tests/fake.py", False),
        (0, 'LOG.warning', "<project_name>/tests/unit/fake.py", True),
        (0, 'LOG.warning', "<project_name>/tests/unit/integrated/fake.py", False))
