...
import <project_name>.api.middleware.auth
from <project_name> import test


class Test<Project_name>KeystoneContextMiddleware(test.TestCase):

    def setUp(self):
        super(Test<Project_name>KeystoneContextMiddleware, self).setUp()

        @webob.dec.wsgify()
        def fake_app(req):
            self.context = req.environ['<project_name>.context']
            return webob.Response()

        self.context = None
        self.middleware = (<project_name>.api.middleware.auth
                           .<Project_name>KeystoneContext(fake_app))
